...
entity ExpenseClaims : cuid, managed {
    ...
    reimbursement   : Association to one Reimbursements on reimbursement.claim = $self;
}
...
entity ExpenseItems : cuid, managed {
    ...
    receipt            : LargeBinary @Core.MediaType : receiptType @Core.ContentDisposition.Filename : receiptName @Core.ContentDisposition.Inline;
...
entity CurrencyView as select from Currency {
    key code : String(3)
}group by code;

entity categoryView as select from ExpensePolicies{
    key  category
}group by category;

entity statusView as select from ExpenseClaims {
    key status
}group by status;